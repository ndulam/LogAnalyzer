 ...
 * @author ${user.name}
 */
object ApacheLogGeneratorSocket {
  ...
  def main(args : Array[String]) {
    ...
    val logDataFeedActor = new ApacheLogGeneratorSocket
    logDataFeedActor.initialize
    logDataFeedActor.LogGenerator(d, n_records_per_day.toInt, port)
  }

}


class ApacheLogGeneratorSocket {

  val str: String = "Hello, How are you"
  private val ipA_by_ctry = Array.ofDim[Int](ApacheLogGeneratorSocket.MAX_IPS_PER_COUNTRY,ApacheLogGeneratorSocket.N_COUNTRIES)
  private val ipB_by_ctry = Array.ofDim[Int](ApacheLogGeneratorSocket.MAX_IPS_PER_COUNTRY,ApacheLogGeneratorSocket.N_COUNTRIES)
  private val tot_ips_by_ctry: Array[Int] = new Array[Int](ApacheLogGeneratorSocket.N_COUNTRIES)
  private val cum_hourly_weight_by_ctry = Array.ofDim[Int](24,ApacheLogGeneratorSocket.N_COUNTRIES)
  ...
  private val requests: Array[String] = new Array[String](ApacheLogGeneratorSocket.MAX_FILE_LINES)
  private val referrers: Array[String] = new Array[String](ApacheLogGeneratorSocket.MAX_FILE_LINES)
  private val user_agents: Array[String] = new Array[String](ApacheLogGeneratorSocket.MAX_FILE_LINES)
  ...
  def initialize: Unit = {
    ...
    for (i <- 0 to ApacheLogGeneratorSocket.N_COUNTRIES - 1) {
      tot_ips_by_ctry(i) = 0
      ctry_ind.put(ctry_abbr(i), String.valueOf(i))
    }

    for (i <- 0 to ApacheLogGeneratorSocket.MAX_IPS_PER_COUNTRY - 1) {
      for (j <- 0 to ApacheLogGeneratorSocket.N_COUNTRIES - 1) {
        ipA_by_ctry(i)(j) = 0
        ipB_by_ctry(i)(j) = 0
      }
    }
    ...
    for (line <- Source.fromInputStream(getClass.getResourceAsStream("/all_classbs.txt")).getLines()) {
      ...
      if (fields.length == 2) {
        ...
        if (i_ctry_s != null) {
          ...
          if (tot_ips_by_ctry(i_ctry) < ApacheLogGeneratorSocket.MAX_IPS_PER_COUNTRY) {
  ...
  def weight: Unit= {
    ...
    var hourly_weight_by_ctry = Array.ofDim[Int](24,ApacheLogGeneratorSocket.N_COUNTRIES)


    for( hour <- 0 to 23){
      for(ctry <- 0 to ApacheLogGeneratorSocket.N_COUNTRIES - 1){
  ...
  def LogGenerator(d: Date, n_records_per_day: Int, port: Int): Unit = {
      ...
      new Thread() {
        override def run(): Unit = {
          ...
          while(true) {
            ...
            clicks_left = 1 + rand.nextInt(ApacheLogGeneratorSocket.MAX_CLICKS_PER_USER)
