...
import com.data.utils.ApacheAccessLog
...
object LogAnalyzerStreamingSQL {
  ...
  def main(args: Array[String]) {
    ...
    val windowDStream = accessLogs.window(WINDOW_LENGTH, SLIDE_INTERVAL)
    ...
    windowDStream.foreachRDD(accessLogs => {
      if (accessLogs.count() == 0) {
        ...
        accessLogs.toDF().registerTempTable("accessLogTable")

        // Calculate statistics based on the content size.
        val contentSizeStats = sqlContext
          .sql("SELECT SUM(contentSize), COUNT(*), MIN(contentSize), MAX(contentSize) FROM accessLogTable")
          .first()
        ...
        val responseCodeToCount = sqlContext
          .sql("SELECT responseCode, COUNT(*) FROM accessLogTable GROUP BY responseCode")
        ...
        val ipAddresses =sqlContext
          .sql("SELECT ipAddress, COUNT(*) AS total FROM accessLogTable GROUP BY ipAddress HAVING total > 10")
          .map(row => row.getString(0))
          .take(100)
        println(s"""IPAddresses > 10 times: ${ipAddresses.mkString("[", ",", "]")}""")

        val topEndpoints = sqlContext
          .sql("SELECT endpoint, COUNT(*) AS total FROM accessLogTable GROUP BY endpoint ORDER BY total DESC LIMIT 10")
